...
export async function fetchWritingFlashCard(): Promise<Word> {
  const res = await fetch('/api/words/writing-flashcard', { credentials: 'include' })
  if (res.status === 404) {
    const body = await res.json().catch(() => ({ error: 'no_cards_due' }))
    throw new ApiError(404, (body as { error?: string }).error ?? 'no_cards_due')
  }
...
export async function submitWritingReview(wordId: string, rating: Rating): Promise<void> {
  const res = await fetch(`/api/words/${wordId}/writing-review`, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ rating }),
